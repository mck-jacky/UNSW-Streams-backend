...
def test_message_id_invalid(initialize):
    ...
    with pytest.raises(InputError):
        message_unreact_v1(user_3, message_id_1, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_3, message_id_2, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_3, message_id_3, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_3, message_id_4, LIKE)
...
def test_invalid_react_id(initialize):
    ...
    with pytest.raises(InputError):
        message_unreact_v1(user_1, message_id_1, 2)
    with pytest.raises(InputError):
        message_unreact_v1(user_1, message_id_2, 3)
    with pytest.raises(InputError):
        message_unreact_v1(user_2, message_id_3, 4)
    with pytest.raises(InputError):
        message_unreact_v1(user_2, message_id_4, 5)

def test_does_not_contain_reacted(initialize):
    # the message does not contain a react with ID react_id from the authorised user
    with pytest.raises(InputError):
        message_unreact_v1(user_1, message_id_1, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_1, message_id_2, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_1, message_id_3, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_1, message_id_4, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_2, message_id_1, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_2, message_id_2, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_2, message_id_3, LIKE)
    with pytest.raises(InputError):
        message_unreact_v1(user_2, message_id_4, LIKE)

def test_unreact_successfully(initialize):
    # message unreact successfully
    assert message_react_v1(user_1, message_id_1, LIKE) == {}
    assert message_unreact_v1(user_1, message_id_1, LIKE) == {}
    assert message_react_v1(user_1, message_id_1, LIKE) == {}
    assert message_unreact_v1(user_1, message_id_1, LIKE) == {}
    assert message_react_v1(user_1, message_id_2, LIKE) == {}
    assert message_unreact_v1(user_1, message_id_2, LIKE) == {}
    assert message_react_v1(user_1, message_id_3, LIKE) == {}
    assert message_unreact_v1(user_1, message_id_3, LIKE) == {}
    assert message_react_v1(user_1, message_id_4, LIKE) == {}
    assert message_unreact_v1(user_1, message_id_4, LIKE) == {}
    assert message_react_v1(user_2, message_id_1, LIKE) == {}
    assert message_unreact_v1(user_2, message_id_1, LIKE) == {}
    assert message_react_v1(user_2, message_id_2, LIKE) == {}
    assert message_unreact_v1(user_2, message_id_2, LIKE) == {}
    assert message_react_v1(user_2, message_id_3, LIKE) == {}
    assert message_unreact_v1(user_2, message_id_3, LIKE) == {}
    assert message_react_v1(user_2, message_id_4, LIKE) == {}
    assert message_unreact_v1(user_2, message_id_4, LIKE) == {}