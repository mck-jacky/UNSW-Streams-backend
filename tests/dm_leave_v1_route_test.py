"""
Server route Pytest for the dm_leave_v1() function.
"""
...
def test_valid_user_no_dms(clear_and_create_new_values):
    ...
    resp = requests.post(config.url + 'dm/leave/v1',
        json = {'token': user4_token, 'dm_id': -1})
    
    assert(resp.status_code == 400)

def test_valid_user_owner_leave_dm(clear_and_create_new_values):
    """
    1. User1 creates DM with user2.
    2. User1 leaves DM with user2.
    3. Assert that response equals expected return.
    """
    ...
    resp2 = requests.get(config.url + 'dm/details/v1',
        params = {'token': fixList[4], 'dm_id': resp0_text})
...
def test_valid_user_member_leave_dm(clear_and_create_new_values):
    """
    1. User1 creates DM with user2.
    2. User2 leaves DM with user1.
    3. Assert that response equals expected return.
    """
    ...
    resp1 = requests.post(config.url + 'dm/leave/v1',
        json = {'token': fixList[4], 'dm_id': resp0_text})
...
def test_valid_user_not_member_of_dm_but_try_leave(clear_and_create_new_values):
    """
    1. User1 creates DM with user2.
    2. User3 tries to leave user1-user2 DM.
    3. Assert that the status code is 403 (AccessError).
    """
    ...
    resp1 = requests.post(config.url + 'dm/leave/v1',
        json = {'token': fixList[5], 'dm_id': resp0_text})
