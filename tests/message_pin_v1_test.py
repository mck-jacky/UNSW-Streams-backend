...
from src.message import message_send_v1, message_senddm_v1, message_pin_v1
...
def test_message_id_invalid(initialize):
    ...
    with pytest.raises(InputError):
        message_pin_v1(user_3, message_id_1)
    with pytest.raises(InputError):
        message_pin_v1(user_3, message_id_2)
    with pytest.raises(InputError):
        message_pin_v1(user_3, message_id_3)
    with pytest.raises(InputError):
        message_pin_v1(user_3, message_id_4)

def test_message_is_already_pinned(initialize):
    # the message is already pinned
    ...
    with pytest.raises(InputError):
        message_pin_v1(user_1, message_id_3)
    message_pin_v1(user_2, message_id_5)
    with pytest.raises(InputError):
        message_pin_v1(user_1, message_id_5)

def test_user_dont_have_permission_to_pin(initialize):
    # message_id refers to a valid message in a joined channel/DM and the authorised user does not have owner permissions in the channel/DM
    with pytest.raises(AccessError):
        message_pin_v1(user_2, message_id_1)
    with pytest.raises(AccessError):
        message_pin_v1(user_3, message_id_5)
    with pytest.raises(AccessError):
        message_pin_v1(user_3, message_id_7)
    with pytest.raises(AccessError):
        message_pin_v1(user_1, message_id_7)

def test_user_pin_successfully(initialize):
